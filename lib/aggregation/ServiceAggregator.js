...
class ServiceAggregator extends BaseGenerator {
	...
	execute() {
		...
		return me._initAggregationConfig().then(
			...
			function onCriticalAggregationError() {

				me.$log( "critical", "abort", "Service Aggregator aborting due to an unrecoverable error!" );
				process.exit( 1 );
		...
		_.each( endpointData, function ( endpoints, path ) {
			...
			_.each( endpoints, function ( endpoint, httpMethod ) {
				...
				if ( lambdaData[ versionHash ] === undefined ) {
					...
					let epDisplayName = httpMethod.toUpperCase() + " " + path;

					me.$log( "warning", "cf.tpl.map.error.missing", "Skipping method mapping for '" + epDisplayName + "'; could not find a valid Lambda function (using 'versionHash':'" + versionHash + "')." );

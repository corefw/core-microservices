...
class BaseExecutionContext extends BaseClass {
	...
	get request() {
		...
		return me.getConfigValue( "request", me._createRequestObject );
	}

	/**
	 * The processed endpoint parameters; pulled from the Request.
	 *
	 * @public
	 * @readonly
	 * @type {Object}
	...
	get parameters() {
		...
		return me.request.parameters;
	}

	/**
	 * Raw parameter information; the structure of this variable will
	...
	/**
	 * The name of the endpoint being executed (e.g. `getManyAddresses`). This
	 ...
	 * (e.g. `sls-service-geographical:getManyAddresses`). This is primarily
	...
	_resolveContextData() {
		...
		rcdi(
			...
			function ( value ) {
				...
				// arn:aws:lambda:us-east-1:277549955817:function:sls-service-competition-dev-getManyActivities
				let spl = value.split( ":" );
	 ...
	 *     contextDataIndex.
	 * @param {string|RegExp} searchPattern - The pattern to use while traversing
	 *     the contextDataIndex in search of a value for the 'dataItem'.
		...
		_.each( index, function ( value, key ) {
			...
			if ( found ) {
				...
				if ( parser === null ) {
					...
					let parserResult = parser( value, key, dataItem, data, index );
	 ...
	 *     returned.
	 * @param {*} [defaultValue] - If provided, and the value could not be found,
	 *     this value will be returned (instead of null).
		...
		_.each( itemKeys, function ( key ) {
			...
			if ( data[ key ] === undefined ) {
				...
				throw new ERRORS.InvalidContextKeyError(
					"Internal Error: A context data lookup attempt referenced a " +
					"context item key ('" + key + "') that is not " +
					"valid or recognized."
				);
			}
	...
	// <editor-fold desc="--- Request Parameter Resolution/Logic -------------">
	...
	_resolveParameters() {
		...
		me.rawParameters = me.initialContextData.event;
	}
	...
	invokeEndpointHandler() {
		...
		// Execute the handler
		return ep.handle( me.request ).then( function ( result ) {

			// Finalize the execution
			return me._succeed( result );
	 ...
	 *
	 * @private
	 * @returns {Request.Request} The new request object
	 */
	_createRequestObject() {

		const me = this;

		return me.$spawn( "microservicesLib", "request/Request", {
			context       : me,
			rawParameters : me.rawParameters,
		} );
	}
	 ...
	 * @returns {Response.BaseResponse} The created response object.
	 */
	...
	_succeed( data ) {
		...
		let responseObject = me._createResponseObject( {
			data: data,
		} );
	...
	_finish( responseObject ) {
		...
		if ( me.hasCallback ) {
			...
			me.$log( "debug", "endpoint.execution.callback", "Executing handler callback..." );
			me.callback( null, formattedResponse );
		}

		// ...and finish up by resolving the promise.
		me.$log( "info", "endpoint.execution.finished", "Endpoint execution complete" );

...
class MetaDeploymentManager extends BaseGenerator {
	...
	execute() {
		...
		me.$log(
			"notice",
			"start",
			"The Metadata Deployment Manager is starting up ..."
		);
				...
				return BB.mapSeries(
					...
					function ( module ) {
						...
						me.$log(
							"info",
							"module.execute",
							"Deferring to Module: " + module.constructor.name
						);
					...
					function () {
						...
						me.$log(
							"notice",
							"end",
							"Metadata deployment complete; exiting."
						);
					}
				);
			}
	 ...
	 *
	 * @access public
	 * @returns {string} Absolute path to the deploy config.
	 */
	 ...
	 * @returns {Object} Deployment configuration data.
	 */
	get deployConfig() {
		...
		try {
			...
			config = me.loadYamlFile( me.deployConfigPath );

		} catch ( err ) {

			me.$throw(
				"deploy.config.syntax.error",
				err,
				"Could not read 'config/meta-deploy.yml'"
			);
		}

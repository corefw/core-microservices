...
class BaseEndpoint extends BaseClass {
	...
	initPathManager() {
		...
		// Add the sub-paths
		pm.setSubPath( "endpointSchemas", 	"endpoint", 		"schemas" 		);
		pm.setSubPath( "endpoints", 		"project", 			"endpoints" 	);
		pm.setSubPath( "projectLib", 		"project", 			"lib" 			);
		pm.setSubPath( "models", 			"projectLib", 		"models" 		);

		// Schemas
		pm.setSubPath( "parameterSchema",	"endpointSchemas", 	"Parameters" 		);
		pm.setSubPath( "pathSchema", 		"endpointSchemas", 	"Paths" 			);
		pm.setSubPath( "responseExample", 	"endpointSchemas", 	"SuccessExample" 	);
		pm.setSubPath( "responseSchema", 	"endpointSchemas", 	"SuccessResponse" 	);
	 ...
	 * used for request/parameter validation within certain contexts.
	 *
	 * @public
	...
	/**
	 * A schema representing the parameters within a valid request, in JSON
	...
	get parameterSchema() {
		...
		return me.getConfigValue( "parameterSchema", me._loadParameterSchema );
	}

	// noinspection JSUnusedGlobalSymbols
	set parameterSchema( /** Object */ val ) {

		const me = this;

		me.setConfigValue( "parameterSchema", val );
	}
	 ...
	 * @returns {Object} The loaded parameter schema.
	 */
	_loadParameterSchema() {
		...
		try {
			...
			return require( me.parameterSchemaPath );
	 ...
	 * swagger specs). So, I refactored the code to build the request schema
	 ...
	 *
	 * @private
	 * @returns {Object} The created request schema.
	 */
	_createRequestSchema() {
		...
		let params	= me.parameterSchema;

		let description = "A request for the " + me.operationId +
			" endpoint/operation.";
		...
		_.each( params, function ( param ) {
			...
			me._createParamSchema( requestSchema, param );
		} );

		if ( me.endpointType === "getMany" ) {

			// Pagination

			requestSchema.properties.pageNumber = {
				"type"        : "integer",
				"format"      : "int32",
				"minimum"     : 1,
				"default"     : 1,
			...
			requestSchema.properties.pageSize = {
				"type"        : "integer",
				"format"      : "int32",
				"minimum"     : 1,
				"maximum"     : 200,
			...
			requestSchema.properties.sort = {
				"type"        : "string",
				"description" : "Fields to sort result by.",
			};
		}
	...
	/**
	 * Converts an OpenAPI Swagger parameter into a JSON Schema object and
	 * injects it into the specified request schema.
	 *
	 * @param {Object} requestSchema - Request schema object.
	 * @param {Object} param - OpenAPI formatted parameter object.
	 * @returns {void}
	 * @private
	 */
	_createParamSchema( requestSchema, param ) {
		...
		_.defaults( requestSchema, {properties: {}} );
		_.defaults( requestSchema, {required: []} );
		...
		if ( paramRequired ) {
			...
			requestSchema.required.push( paramName );
		}
	}
	 ...
	 *
	 * @see _createResponseSchema <- does not exist?
	 * @public
	 * @type {Object}
	 * @readonly
	 */
	get requestSchema() {
		...
		return me.getConfigValue( "requestSchema", me._createRequestSchema );
	}
	...
	/**
	 * A schema representing a valid response, in JSON Schema object format.
	 *
	 * @public
	...
	get responseSchema() {
		...
		return BB.resolve( me.getConfigValue( "responseSchema" ) );
	}

	// noinspection JSUnusedGlobalSymbols
	set responseSchema( /** Object */ val ) {

		const me = this;

		me.setConfigValue( "responseSchema", val );
	}
	...
	_loadResponseSchema() {
		...
		// let responseSchema = require( me.responseSchemaPath );
		//
		// return REF_PARSER.bundle( responseSchema, me._refParserConfig );
		// } );
		...
		// try {
		//
		// 	return require( me.responseSchemaPath );
		//
		// } catch ( err ) {
		//
		// 	throw new ERRORS.MissingResponseSchemaError(
	...
	execute( event, context, callback ) {
		...
		return BB.try( function () {
			...
			// Prepare...
			let executionContext = me._prepForExecution( event, context, callback );
	 ...
	 *     after the endpoint has executed successfully (or failed with an
	 *     error)
	 * @returns {ExecutionContext.BaseExecutionContext} The environment specific
	 *     execution context.
	 */

...
class BaseEndpoint extends BaseClass {
	...
	_initKmsCryptConfig() {
		...
		// Determine when the configuration was last applied...

		let tsMaxElapsed	= 60 * 60 * 2; // 2 Hours
		let tsNow			= Math.floor( Date.now() / 1000 ); // UTC timestamp in seconds
		let tsConfigApplied	= process.env.KMS_CRYPT_CONFIG_APPLIED || 0;
		let tsElapsed		= tsNow - tsConfigApplied;

		me.$log(
			"info",
			"kms.crypt.config.elapsed.max",
			"KMS Crypt Config max elapsed time (seconds): " + tsMaxElapsed
		);

		me.$log(
			"info",
			"kms.crypt.config.elapsed.current",
			"KMS Crypt Config current elapsed time (seconds): " + tsElapsed
		);

		// The configuration has been applied within the max time limit, exit early.

		if ( tsElapsed < tsMaxElapsed ) {

			me.$log(
				"info",
				"kms.crypt.config",
				"KMS Crypt Config applied to environment within max elapsed time limit, skipping."
			);
		...
		return BB.try( function () {
			...
			process.env.KMS_CRYPT_CONFIG_APPLIED = tsNow;

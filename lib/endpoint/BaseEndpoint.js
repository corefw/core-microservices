...
const DecryptConfig	= require( "@c2cs/kms-crypt-config" ).DecryptConfig;
...
class BaseEndpoint extends BaseClass {
	...
	_initConfig() {
		...
		return BB.try( function () {
			...
			let decryptConfig = new DecryptConfig();

			return decryptConfig.decryptAndApply();
	...
	initPathManager() {
		...
		pm.setSubPath( "pathSchema", 		"endpointSchemas", 	"Paths.json" 			);
		pm.setSubPath( "responseExample", 	"endpointSchemas", 	"SuccessExample.json" 	);
		pm.setSubPath( "responseSchema", 	"endpointSchemas", 	"SuccessResponse.json" 	);
	...
	_createRequestSchema() {
		...
		let paramSchema	= me.parameterSchema;
		let params		= paramSchema.parameters;
		...
		// Iterate over each parameter and add it to
		// the request schema's "properties" object.
	...
	get responseSchema() {
		...
		return me.getConfigValue( "responseSchema", me._loadResponseSchema );
	}
	 ...
	 *     loaded (for any reason).
	 * @returns {Object} The loaded response schema.
	 */
	_loadResponseSchema() {

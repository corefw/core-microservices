...
class BaseEndpoint extends BaseClass {
	...
	_createRequestSchema() {
		...
		return BB.try( function () {
			...
			let requestSchema = {
				...
				"type"        : "object",
				"description" : "A request for the " + me.operationId + " endpoint/operation.",
				"properties"  : {},
			};
	...
	/**
	 * Get a schema representing an error response, in JSON Schema object format.
	 *
	 * @public
	 * @throws {Errors.MissingResponseSchemaError} If the schema is requested
	 *     but is not defined.
	 * @returns {Promise<Object>} Error response schema.
	 */
	getErrorResponseSchema() {
		...
		if ( me.hasConfigValue( "errorResponseSchema" ) ) {

			return BB.resolve(
				me.getConfigValue( "errorResponseSchema" )
			);
		}

		return me._loadErrorResponseSchema()
			.then( function ( errorResponseSchema ) {

				me.setConfigValue( "errorResponseSchema", errorResponseSchema );
	...
	/**
	 * Set a schema representing an error response, in JSON Schema object format.
	 *
	 * @public
	 * @param {Promise<Object>} errorResponseSchema - Error response schema.
	 * @returns {void}
	 */
	setErrorResponseSchema( errorResponseSchema ) {
	...
	/**
	 * Loads the common error response schema (from a file).
	 *
	 * @private
	 * @throws {Errors.MissingResponseSchemaError} If the schema could not be
	 *     loaded (for any reason).
	 * @returns {Promise<Object>} The loaded error response schema.
	 */
	_loadErrorResponseSchema() {
		...
		let schemaPath = PATH.join(
			schemaGenerator.commonSchemaDefinitionRoot,
			"/response/ErrorResponse.yml"
		);

		return schemaGenerator.buildSchema( schemaPath )
			.catch( function ( err ) {

				throw new ERRORS.MissingResponseSchemaError(
					err,
					"Failed to load the error response schema."
				);
			} );
	}
	...
	handle( request ) {
		...
		return BB.try( function () {
			...
			me.$log(
				"info",
				"Session validation succeeded; continuing."
			);

			return me._handle( request );
		} );
	}
	 ...
	 *
	 * Executes associated model operation based on endpoint type.
	 *
	 * @private
	 * @param {Request.Request} request - A fully resolved request object that
	 *     describes the request being made to the endpoint.
	...
	_handle( request ) {
		...
		return BB.try( function () {
			...
			let model			= me.model;
			let modelMethod		= model[ endpointType + "ByRequest" ].bind( model );
	...
	_resolveEnvironment( contextData ) {
		...
		} else if ( _.get( contextData.event, "requestContext.apiId" ) !== undefined ) {
	...
	_parseParameters( request ) { // eslint-disable-line no-unused-vars
		...
		// FIXME: should this be declared as @template instead of @abstract?
	 ...
	 */
	checkAccess( request ) { // eslint-disable-line no-unused-vars

		// The default behavior is to allow execution.
		// Child classes should override this method
		// if they need to apply ACM logic.

		// FIXME: throw error, in order to force child classes to always implement?
	}

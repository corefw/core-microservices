...
class ServerlessConfigGenerator extends BaseGenerator {
	 ...
	 * `serverless-function.yml` configuration files.
	 *
	 * @protected
	...
	_createSpecialVariables( vars, epDetailsObj ) {
		...
		// Add the special variable: ${endpointRequestTemplate}
	 ...
	 * `serverless-function.yml` config file via the `${endpointResponse}`
	 * variable identifier.
	 *
	...
	_buildEndpointResponseTemplate() {
		...
		return me._loadCommonConfigFile(
			"endpoint-response.yml", "endpointResponse"
		);
	}

	/**
	 * This method builds an object that endpoints can use in their
	 * `serverless-function.yml` config file via the
	...
	_buildEndpointParameterTemplate( epDetailsObj ) {
		...
		// defined in the endpoint's Parameters schema file.

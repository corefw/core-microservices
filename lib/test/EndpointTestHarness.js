...
class EndpointTestHarness extends BaseClass {
	...
	/**
	 * Executed before each test is run. Modifies configuration data.
	 *
	...
	beforeTest( cfg ) {
		...
		let logger = me.logger.fork( {
			component  : "testHarness.beforeTest",
			namePrefix : "before.test",
		} );

		return BB.try( function () {

			logger.info( "Checking for 'beforeTest' operations in test configuration." );

			let operations	= _.get( cfg, "beforeTest", null );

			// Nothing to do, return
			if ( _.isNil( operations ) ) {

				logger.info( "No 'beforeTest' operations defined in test configuration." );

				return BB.resolve();
			}

			logger.info(
				`Found [${ operations.length }] 'beforeTest' operation(s) defined in test configuration.`
			);
			...
			// Process any beforeTest operations
			return BB.each( operations, function ( operation, index ) {

				logger.info(
					`Processing 'beforeTest' operation [${ index + 1 }/${ operations.length }]...`
				);
				...
				let responseMap		= _.get( operation, "responseMap", null );
				...
				return endpoint.execute( event, context )
					.then( function ( response ) {
						...
						if ( responseMap ) {
							...
							logger.info( "Applying response map to test configuration." );

							_.each( responseMap, function ( item ) {

								let value = _.get( response, item.from, null );

								_.set( cfg, item.to, value );
							} );
						}
					} );
			} );
			...
			// Process any custom beforeTest()

			const beforeTest = me.getConfigValue( "beforeTest" );

			if ( beforeTest ) {

				logger.info( "Running custom defined beforeTest();" );

				return beforeTest( me, cfg );
			}
	...
	afterTest( cfg, responseData ) {
		...
		let logger = me.logger.fork( {
			component  : "testHarness.afterTest",
			namePrefix : "after.test",
		} );

		return BB.try( function () {

			logger.info( "Checking for 'afterTest' operations in test configuration." );

			let operations	= _.get( cfg, "afterTest", null );

			// Nothing to do, return
			if ( _.isNil( operations ) ) {

				logger.info( "No 'afterTest' operations defined in test configuration." );

				return BB.resolve();
			}

			logger.info(
				`Found [${ operations.length }] 'afterTest' operation(s) defined in test configuration.`
			);
			...
			// Process any afterTest operations
			return BB.each( operations, function ( operation, index ) {

				logger.info(
					`Processing 'afterTest' operation [${ index + 1 }/${ operations.length }]...`
				);
				...
				if ( responseMap ) {
					...
					logger.info( "Applying response map to operation." );

					_.each( responseMap, function ( item ) {

						let value = _.get( responseData.response, item.from, null );

						_.set( operation, item.to, value );
					} );
				}
			...
			// Process any custom afterTest()

			const afterTest = me.getConfigValue( "afterTest" );

			if ( afterTest ) {

				logger.info( "Running custom defined afterTest();" );

				return afterTest( me, responseData );
			}
	...
	execAndValidate( cfg ) {
		...
		return BB.try( function () {
			...
			return me.afterTest( cfg, responseData );
	...
	_initKmsCryptConfig() {
		...
		return BB.try( function () {
			...
			return BB.try( function () {
				...
				me.$log( "info", "KmsCrypt Config applied successfully." );

				timerStop = new Date().getTime();

				me.$log(
					"info",
					"KmsCrypt Config Application Time (milliseconds): " + ( timerStop - timerStart )
				);
			} );
		} );
	}

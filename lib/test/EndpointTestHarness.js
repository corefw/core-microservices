...
class EndpointTestHarness extends BaseClass {
	...
	_initKmsCryptConfig() {
		...
		return BB.try( function () {
			...
			// KMS Crypt Config already applied, skip...

			if ( process.env.KMS_CRYPT_CONFIG_APPLIED ) {

				me.$log(
					"info",
					"kms.crypt.config.applied",
					"KMS Crypt Config already applied, skipping."
				);
			...
			if ( config.env ) {
				...
				_.assign( process.env, config[ testEnvironment ].environment );
			}
			...
			me.$log(
				"info",
				"kms.crypt.config.applied",
				"KMS Crypt Config applied successfully."
			);

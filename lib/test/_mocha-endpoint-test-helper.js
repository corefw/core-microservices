...
_utils.paths.projectConfig 	= PATH.join( _utils.paths.project, 		"config" 								);
...
_utils.paths.testLib		= PATH.join( _utils.paths.toolsLib, 	"test" 									);
 ...
 */
_utils.loadYamlFile = function ( absPath, property ) {

	return EndpointTestHarness.loadYamlFile( absPath, property );
};

_utils.loadKmsCryptConfig = function () {

	let kmsCryptConfigPathAbs =
		PATH.join( _utils.paths.projectConfig, "kmscrypt.yml" );

	return _utils.loadYamlFile( kmsCryptConfigPathAbs );
};

_utils.getEndpointClass = function ( endpointPathRel ) {

	let pathParts		= endpointPathRel.split( "/" );
	let endpointName	= pathParts[ pathParts.length - 1 ];

	let endpointClassPath =
		PATH.join( _utils.paths.endpoints, endpointPathRel, endpointName + ".js" );

	return require( endpointClassPath );
};
 ...
 * @param {string} cfg.endpointDirectoryPath - The absolute path of the endpoint
 *     directory.

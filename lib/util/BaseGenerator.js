...
class BaseGenerator extends BaseClass {
	...
	initialize( cfg ) {
		...
		if ( !_.isPlainObject( cfg ) || !_.isString( cfg.serviceRootPath ) ) {
			...
			throw new Error(
				"All generators require the 'serviceRootPath' configuration property to be " +
				"defined when being instantiated."
			);
		}
	 ...
	 * core-microservices, based on the generators current `configVariant` setting
	 * (e.g. `serverless/config/default`).
	 *
	 * @public
	 * @type {string}
	 * @readonly
	...
	get commonConfigRoot() {
		...
		return PATH.join( me.toolsLibRoot, "serverless/config", me.configVariant );
	}

	/**
	 * The absolute path to the common schemas directory in
	 * core-microservices (`schemas`).
	 *
	 * @public
	 * @type {string}
	 * @readonly
	...
	get commonSchemaRoot() {
		...
		return PATH.join( me.toolsLibRoot, "schemas" );
	}

	/**
	 * The absolute path to the common schema definitions directory
	 * in core-microservices (`schemas/definitions`).
	 *
	 * @public
	 * @type {string}
	 * @readonly
	 ...
	 * common configs/schemas/etc that this generator will use when
	...
	/**
	 * Reads the contents of a "common JSON file" (via `require()`)
	...
	_loadCommonConfigFile( relPath, property ) {
		...
		return me._loadJsonFile( absPath, property );
	}

	/**
	 * Reads the contents of a "common schema file" (via `require()`)
	 * and returns it as an object.
	 *
	 * @protected
	 * @param {string} relPath - The relative path of the file to read; this
	...
	getServiceEndpoints( force ) {
		...
		if ( force || me._endpointDetailsCache === undefined ) {
			...
			// named 'serverless-function.json'
			let pattern			= /serverless-function\.json/;
			let epConfigFiles	= me._findFilesWithPattern( me.serviceEndpointPath, pattern );
	 ...
	 * the path to that endpoint's `serverless-function.json` file.
	 *
	 * @private
	 * @param {string} configPath - The absolute path to an endpoint's
	 *     `serverless-function.json` configuration file.
	...
	_resolveEndpointDetails( configPath ) {
		...
		ret.schemaRootPath = PATH.join( ret.endpointRoot, "schemas" );

		// Parameter config
		ret.parameterConfigPath = PATH.join( ret.schemaRootPath, "Parameters.json" );
		ret.parameterConfig = me._loadParameterConfig( ret );

		// Path config
		ret.pathConfigPath = PATH.join( ret.schemaRootPath, "Paths.json" );
	 ...
	 * 'function-defaults.json', and will be used to fill in any values that
	 * a loaded 'serverless-function.json' file does not define.
	 *
	 * @public
	 * @type {Object}
	 * @readonly
	...
	get functionDefaults() {
		...
		if (
			...
			me._functionDefaults =
				me._loadCommonConfigFile( "function-defaults.json" );
		}
	 ...
	 * config file, 'http-event-defaults.json', and will be used to fill in any
	...
	get httpEventDefaults() {
		...
		if (
			...
			me._httpEventDefaults =
				me._loadCommonConfigFile( "http-event-defaults.json" );
		}
	...
	/**
	 * A plain object containing default values for 'http-shared' events within
	 * function configurations; these defaults will be loaded from the common
	 * config file, 'http-shared-event-defaults.json', and will be used to fill
	 * in any values that any http-shared events declarations do not define.
	 *
	 * Important Note: the 'http-shared' event type is not an official
	 * Serverless event type but is, rather, one that is processed by a
	 * Serverless plugin ('apigateway', which we wrote).
	 *
	 * @public
	 * @type {Object}
	 * @readonly
	 */
	get httpSharedEventDefaults() {
		...
		// Read from cache or load the config file...
		if (
			me._httpSharedEventDefaults === undefined ||
			me._httpSharedEventDefaults === null
		) {

			me._httpSharedEventDefaults =
				me._loadCommonConfigFile( "http-shared-event-defaults.json" );
		}

		// Out of paranoia, we clone the default config
		// to ensure that it cannot be modified by
		// subsequent processing.
		return _.cloneDeep( me._httpSharedEventDefaults );
	}

	/**
	 * A plain object containing default values for 'http-shared' events that
	 * are automatically created by paths found within an endpoint's Paths.json
	...
	get httpSharedEventScaffold() {
		...
		// Read from cache or load the config file...
		if (
			me._httpSharedEventScaffold === undefined ||
			me._httpSharedEventScaffold === null
		) {

			me._httpSharedEventScaffold =
				me._loadCommonConfigFile( "http-shared-event-scaffold.json" );
		}

		// Out of paranoia, we clone the default config
		// to ensure that it cannot be modified by
		// subsequent processing.
		return _.cloneDeep( me._httpSharedEventScaffold );
	}

	/**
	 * This helper method assists `_resolveEndpointDetails()` by loading
	 * the `serverless-function.json` configuration file and parsing it
	...
	_loadEpFunctionConfig( epDetailsObj ) {
		...
		// Read the serverless-function.json file...
		let cfg = require( epDetailsObj.functionConfigPath );
		...
		// to be injected into the `serverless-function.json` file
		...
		// Allows child classes the opportunity to inject special,
		// context-specific, variables, prior to reading the function config.
		// (This is really only useful, in this context, to the
		// `ServerlessConfigGenerator` class, but it was added here to allow
		// re-use of the _createSpecialVariables method in other contexts)
		me._createSpecialVariables( parseVars, epDetailsObj );

		// If the provided function has an 'Paths.json' configuration,
		// then we will ensure that each path within it is automatically
		// declared as an 'http-shared' event.
		cfg = me._addAutomaticEventsToFunction( cfg, epDetailsObj );

		// Apply event defaults
		cfg = me._applyEventDefaults( cfg );

		// Parse the config object from `serverless-function.json`, injecting
		...
		_.each( cfg.events, function ( eventWrapper ) {
			...
			_.each( eventWrapper, function ( ev, evType ) {
				...
				switch ( evType.toLowerCase() ) {
					...
					case "http-shared":
						newConfig = _.merge( ev, me.httpSharedEventDefaults );
						break;
	 ...
	 * sources (such as the endpoint's Paths.json schema).
	 *
	 * @private
	...
	_addAutomaticEventsToFunction( cfg, epDetailsObj ) {
		...
		// If the endpoint does not have a Paths.json schema, then
		// we won't add any events automatically, so we can exit early...
		if ( epDetailsObj.pathConfig === undefined || epDetailsObj.pathConfig === null ) {
		...
		_.each( epDetailsObj.pathConfig, function ( pathSchema, pathName ) {
			...
			// defined within a http or http-shared event.
			if ( !me._functionConfigHasPath( cfg, pathName, [ "http", "http-shared" ] ) ) {

				// Generate the new event
				let newEvent = me._buildHttpSharedEvent( pathName, pathSchema );
	...
	/**
	 * Constructs a 'http-shared' event configuration object for a given
	 * path schema (from within an endpoint's Paths.json schema)
	 *
	 * @private
	 * @param {string} pathName - The path from a Paths.json schema file
	 * @param {Object} pathSchema - The full schema for the path.
	 * @returns {Object} A 'http-shared' event...
	 */
	_buildHttpSharedEvent( pathName, pathSchema ) {
		...
		let ret = me.httpSharedEventScaffold;
	 ...
	 * to be used within an 'http' or 'http-shared' event as part of
	 * a Serverless function config.
	 *
	 * One of the main uses for this method is the conversion of Swagger
	...
	/**
	 * This helper method assists `_resolveEndpointDetails()` by loading
	 * the `Parameters.json` schema for a single endpoint, if it exists.
	 *
	 * @private
	 * @param {Object} epDetailsObj - A special endpoint details object that
	 *     is constructed by `_resolveEndpointDetails()`
	 * @returns {?Object} The contents of `Parameters.json` or NULL if the
	...
	_loadParameterConfig( epDetailsObj ) {
		...
		try {
			...
			let tmp = require( epDetailsObj.parameterConfigPath );
	...
	/**
	 * This helper method assists `_resolveEndpointDetails()` by loading
	 * the `Paths.json` schema for a single endpoint, if it exists.
	 *
	 * @private
	 * @param {Object} epDetailsObj - A special endpoint details object that
	 *     is constructed by `_resolveEndpointDetails()`
	 * @returns {?Object} The contents of `Paths.json` or NULL if the
	 *     file was not found for the endpoint specified in `epDetailsObj`.
	 */
	_loadPathConfig( epDetailsObj ) {
		...
		try {
			...
			let tmp = require( epDetailsObj.pathConfigPath );
	 ...
	 * especially objects that are loaded from JSON files.
	 *
	 ...
	 * since relevant JSON files will be passed, directly, into this method.
	 *
	 * @protected
	 ...
	 * @param {RegExp} pattern - The regular expression to test each absolute path against.

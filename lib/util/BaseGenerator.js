...
class BaseGenerator extends BaseClass {
	...
	_resolveEndpointDetails( configPath ) {
		...
		ret.parameterConfigPath = PATH.join(
			ret.schemaRootPath, "Parameters" //.json or .js acceptable
		);
		...
		ret.pathConfigPath = PATH.join(
			ret.schemaRootPath, "Paths" //.json or .js acceptable
		);
	 ...
	 * are automatically created by paths found within an endpoint's Paths(.json/.js)
	...
	_loadEpFunctionConfig( epDetailsObj ) {
		...
		// If the provided function has an 'Paths(.json/.js)' configuration,
	 ...
	 * sources (such as the endpoint's Paths(.json/.js) schema).
	 *
	 * @private
	...
	_addAutomaticEventsToFunction( cfg, epDetailsObj ) {
		...
		// If the endpoint does not have a Paths(.json/.js) schema, then
	 ...
	 * path schema (from within an endpoint's Paths(.json/.js) schema)
	 *
	 * @private
	 * @param {string} pathName - The path from a Paths(.json/.js) schema file
	...
	/**
	 * This helper method assists `_resolveEndpointDetails()` by loading
	 * the `Parameters(.json/.js)` schema for a single endpoint, if it exists.
	 *
	 * @private
	 * @param {Object} epDetailsObj - A special endpoint details object that
	 *     is constructed by `_resolveEndpointDetails()`
	 * @returns {?Object} The contents of `Parameters(.json/.js)` or NULL if the
	...
	/**
	 * This helper method assists `_resolveEndpointDetails()` by loading
	 * the `Paths(.json/.js)` schema for a single endpoint, if it exists.
	 *
	 * @private
	 * @param {Object} epDetailsObj - A special endpoint details object that
	 *     is constructed by `_resolveEndpointDetails()`
	 * @returns {?Object} The contents of `Paths(.json/.js)` or NULL if the
	 *     file was not found for the endpoint specified in `epDetailsObj`.
	 */

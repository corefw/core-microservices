...
class SchemaGenerator extends BaseGenerator {
		...
		return {
			...
			resolve: {
				...
				customResolver: {
					...
					canRead: /^(common|endpoints?|service|model):/i,
	...
	_customRefResolver( file ) {
		...
		} else if ( _.startsWith( url, "model:" ) ) {
		...
		if ( refType !== null ) {
			...
			try {
				...
				console.log( "Resolver Method: " + resolverMethod );
	...
	_resolveEndpointRef( path ) {
		...
		Valid Reference Examples
		------------------------
		"$ref" : "endpoint:/ReadManyActivities/SuccessResponse"
		*/
	...
	/**
	 * A specialist resolver for $ref values that start with "model:".
	 * This method is a helper for, and is called exclusively by, the
	 * `_customRefResolver` method.
	 *
	 * $refs of the "model" type will load model-specific schema files
	 * from the `schema` sub-directory within the model's root directory.
	 *
	 * @see _customRefResolver
	 * @private
	 * @param {string} path - The $ref value, with the $ref type removed.
	 * @returns {Object} The resolved schema object.
	 */
	_resolveModelRef( path ) {

		/*
		Valid Reference Examples
		------------------------
		"$ref" : "model:/Activity/Resource"
		*/
		...
		// Split up the 'path' so that we can resolve
		// and remove the model name from it.
		let spl = path.split( "/" );

		// Capture the model name...
		let modelName = spl.shift();

		if ( _.isNil( modelName ) || modelName === "" ) {

			modelName = spl.shift();
		}

		// The remaining path parts constitute a relative
		// path within the model's `schema` directory
		let relPath = spl.join( "/" );

		let modelRootPathAbs	= me.serviceModelRootPath;
		let modelSchemaPathAbs	= PATH.join( modelRootPathAbs, modelName, "/schema", relPath + ".yml" );

		// Load the file and return it as an object
		return me._loadSchema( modelSchemaPathAbs );
	}
	...
	_buildEndpointPaths() {
		...
		_.each( epd, function ( data ) {
			...
			if ( _.isPlainObject( data.pathConfig ) ) {
				...
				_.each( data.pathConfig, function ( pathData, pathName ) {
					...
					if ( _.isUndefined( paths[ pathName ] ) ) {

						paths[ pathName ] = {};
					}

					_.assign( paths[ pathName ], pathData );
				} );
			}
		} );
		...
		let pathNames = _.keys( paths ).sort();

		paths = _.pick( paths, pathNames );

		// Sort by CRUD operation order

		let operationOrder = [
			"post",		// Create
			"get",		// Read
			"head",		// Read Meta Information
			"patch",	// Update
			"delete",	// Delete
			"put",		// Replace - Not Supported by JSON:API
			"options",	// ???
			"trace",	// ???
		];

		paths = _.mapValues( paths, function ( path ) {

			let operations = _.keys( path );

			operations = _.sortBy( operations, function ( operation ) {

				return _.indexOf( operationOrder, operation	);
			} );

			return _.pick( path, operations );
		} );

...
/**
 * This class is used in the automatic generation of schemas.
 *
 * @memberOf Util
...
class SchemaGenerator extends BaseGenerator {
	 ...
	 * the `core-microservices` library's `schemas/definitions` directory.
	 *
	 * @see _customRefResolver
	 * @private
	 * @param {string} path - The $ref value, with the $ref type removed.
	...
	_resolveCommonRef( path ) {
		...
		*/
		...
		try {
			...
			absPath = PATH.join( me.commonSchemaDefinitionRoot, path ); // + ".json";
	 ...
	 *
	 * @public
	 * @param {string} rootSchema - Root schema file.
